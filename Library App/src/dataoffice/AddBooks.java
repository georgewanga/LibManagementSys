...
public class AddBooks extends JInternalFrame {
    ...
    private final String[] informationString = {" The book serial: ", " The book title: ",
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                if (isCorrect()) {
                    Thread runner = new Thread() {
                        ...
                        @Override
                        public void run() {
                            String Query = "SELECT * FROM " + CommonVarriables.tablesToCreate[2]
                                    + " WHERE " + CommonVarriables.columnsToAdd[3][0] + " = '" + data[0] + "'";
                            ...
                            if (!(data[0]).equalsIgnoreCase(Serial)) {
                                try {
                                    String sql = "INSERT INTO " + CommonVarriables.tablesToCreate[2]
                                            + " (" + CommonVarriables.COLS_AvailableBooks + ") "
                                            + "VALUES (?,?,?,?,?,?,?,'" + CommonVarriables.dateToday + "',?)";
                                    PreparedStatement ps = CommonVarriables.getPreparedStatement(sql);
