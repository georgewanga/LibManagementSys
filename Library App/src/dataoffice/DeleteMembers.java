        ...
        insertInformationButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                if (isCorrect()) {
                    ...
                    String Query1 = "SELECT * FROM " + usersTable + " WHERE " + login + " = 'active' and " + CommonVarriables.columnsToAdd[0][8]
                            + " =  '" + data[2] + "'";
                    String Query2 = "SELECT * FROM " + usersTable + " WHERE " + CommonVarriables.columnsToAdd[0][7] + " = '" + data[0] + "'";
                    String Query3 = "SELECT * FROM " + usersTable + " WHERE " + login + " = 'active'";

                    //for checking the password 
                    member = new Members();
                    member.connection(Query1);
                    ...
                    if (Password.trim().equalsIgnoreCase("Available")) {
                            ...
                            Thread runner = new Thread() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        String nom = member.getFirstName() + " " + member.getSurame()
                                                + " " + member.getMiddleName();

                                        String Query4 = "SELECT * FROM " + CommonVarriables.tablesToCreate[1];
                                        int ID = 0;
                                        ResultSet resultSet = new CommonVarriables().getResultSet(Query4);
                                        while (resultSet.next()) {
                                            ID++;
                                        }
                                        //ID = ID;

                                        String sql1 = "INSERT INTO " + CommonVarriables.tablesToCreate[1] + " ("
                                                + CommonVarriables.COLS_DELETING_users + ") SELECT " + CommonVarriables.COLS_DELETING_users
                                                + " FROM " + usersTable + " WHERE " + CommonVarriables.columnsToAdd[0][7] + " = '" + data[0] + "'";
                                        String sql2 = "UPDATE " + CommonVarriables.tablesToCreate[1] + " SET " + CommonVarriables.columnsToAdd[8][1]
                                                + " = '" + CommonVarriables.dateToday + "', " + CommonVarriables.columnsToAdd[8][2] + " = '"
                                                + nom + "', " + CommonVarriables.columnsToAdd[8][3] + " = '" + data[1] + "' WHERE "
                                                + CommonVarriables.columnsToAdd[0][7] + " = '" + data[0] + "'";
                                        String sql3 = "DELETE FROM " + usersTable + " WHERE ID = '" + ID + "'";
                                        ...
                                        Logger.getLogger(DeleteMembers.class.getName()).log(Level.SEVERE, null, ex);
                                    }
                                }
                            };
                            runner.start();
                        ...
                        JOptionPane.showMessageDialog(null, "Wrong Password !! Ensure You Are Logged into your own Account", "Error", JOptionPane.ERROR_MESSAGE);
